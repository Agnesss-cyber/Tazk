...
import { getWorkspaceById, getBoards, getColumns, updateBoard, addColumn, addTask } from '../api/auth'
...
function BoardColumn({ column, onAddTask }) {
  ...
  async function handleSaveTask() {
    ...
    setForm({ title: '', description: '', effort: '', urgency: '' })
    setIsAdding(false)
    setIsSaving(false)
  }
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Box sx={{
            ...
            bgcolor: COLUMN_COLORS[column.name] || '#64748b',
          }} />
          <Typography sx={{ fontSize: '0.88rem', fontWeight: 700, color: '#f1f5f9' }}>
            {column.name}
          </Typography>
        </Box>
        <Typography sx={{
          fontSize: '0.75rem', fontWeight: 600, color: '#64748b',
          bgcolor: 'rgba(255,255,255,0.05)', px: 1.2, py: 0.3, borderRadius: '6px',
        }}>
      ...
      <Box sx={{ flex: 1, p: 1.5, minHeight: 150, overflowY: 'auto' }}>
        {column.tasks?.map((task) => (
          <Box key={task.id} sx={{
            ...
            {/* Effort / Urgency badges */}
            {(task.effort || task.urgency) && (
              <Box sx={{ display: 'flex', gap: 0.8, mt: 1, flexWrap: 'wrap' }}>
                {task.effort && (
                  <Box sx={{
                    fontSize: '0.68rem', fontWeight: 600, px: 1, py: 0.3,
                    borderRadius: '4px', textTransform: 'capitalize',
                    bgcolor: task.effort === 'HIGH' ? 'rgba(239,68,68,0.12)'
                           : task.effort === 'MEDIUM' ? 'rgba(245,158,11,0.12)'
                           : 'rgba(100,116,139,0.12)',
                    color: task.effort === 'HIGH' ? '#f87171'
                         : task.effort === 'MEDIUM' ? '#fbbf24'
                         : '#94a3b8',
                  }}>
                    {task.effort.toLowerCase()} effort
                  </Box>
                )}
                {task.urgency && (
                  <Box sx={{
                    fontSize: '0.68rem', fontWeight: 600, px: 1, py: 0.3,
                    borderRadius: '4px', textTransform: 'capitalize',
                    bgcolor: task.urgency === 'HIGH' ? 'rgba(239,68,68,0.12)'
                           : task.urgency === 'MEDIUM' ? 'rgba(59,130,246,0.12)'
                           : 'rgba(100,116,139,0.12)',
                    color: task.urgency === 'HIGH' ? '#f87171'
                         : task.urgency === 'MEDIUM' ? '#60a5fa'
                         : '#94a3b8',
                  }}>
                    {task.urgency.toLowerCase()} urgency
                  </Box>
                )}
              </Box>
            )}
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <FormControl size="small" fullWidth>
                <InputLabel sx={{ fontSize: '0.78rem', color: '#64748b', '&.Mui-focused': { color: '#3b82f6' } }}>
                  Effort
                </InputLabel>
                <Select
                  value={form.effort}
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <IconButton
                size="small" onClick={handleSaveTask}
                disabled={isSaving || !form.title.trim()}
                sx={{
                  bgcolor: '#3b82f6', borderRadius: '8px', color: '#fff',
              ...
              <IconButton
                size="small" onClick={handleCancel} disabled={isSaving}
...
export default function Board() {
  ...
  async function fetchData() {
    try {
      setLoading(true)
      const [ws, allBoards] = await Promise.all([
  ...
  // Called from Sidebar when a new board is created
  ...
  return (
    ...
    >
      <Box sx={{
        ...
        height: 'calc(100vh - 120px)', overflowX: 'auto', pb: 2,
      }}>
        ...
        <Box sx={{ flexShrink: 0 }}>
          {!addingColumn ? (
            ...
            <Box sx={{
              ...
              <Box sx={{ display: 'flex', gap: 1 }}>
                <IconButton
                  size="small" onClick={handleAddColumn}
                ...
                <IconButton
                  size="small" onClick={() => { setAddingColumn(false); setNewColumnName('') }}
                  sx={{ bgcolor: 'rgba(255,255,255,0.06)', borderRadius: '8px', color: '#64748b', '&:hover': { color: '#f1f5f9' } }}
                >
                  <CloseIcon sx={{ fontSize: 16 }} />
                </IconButton>
